    ...
    try {
        ...
        foreach ($groupRef in $groupRefs) {
            try {
                ...
                Write-Host "⚠️ Could not retrieve group details for ID: $($groupRef.Id)"
            }
        }

        Write-Host "🔍 Retrieved $($groups.Count) groups for model user."
        ...
        if ($addedGroups.Count -gt 0) {
            $groupSummary += " Added to groups: " + ($addedGroups -join ", ") + "."
        }
        if ($skippedGroups.Count -gt 0) {
            $groupSummary += " Skipped mail-enabled groups: " + ($skippedGroups -join ", ") + "."
        }
        if ($groupSummary) {
            $message += $groupSummary
            Write-Host "📦 Group summary: $groupSummary"
        }
